...
impl Header {
    fn size(&self) -> u64 {
        10 + // Raw header.
            self.ext.as_ref().map(|x| x.size()).unwrap_or(0)
    }

    fn frame_bytes(&self) -> u64 {
        u64::from(self.tag_size) - self.ext.as_ref().map(|x| x.size()).unwrap_or(0)
    }
...
impl Header {
    fn decode(mut reader: impl io::Read) -> crate::Result<Header> {
        ...
        let (ver_major, ver_minor) = (header[4], header[3]);
        ...
        let maybe_unsynch_u32: Box<dyn Fn(u32) -> u32> = if flags.contains(Flags::UNSYNCHRONISATION)
        {
            Box::new(unsynch::decode_u32)
        } else {
            Box::new(|i| i)
        };

        let ext = if flags.contains(Flags::EXTENDED_HEADER) {
            let mut ext_header = [0; 10];
            let nread = reader.read(&mut ext_header)?;
            if nread < ext_header.len() {
                return Err(Error::new(
                    ErrorKind::Parsing,
                    "unexpected EOF while reading extended header",
                ));
            }

            // The extended header size excludes, the 4 bytes encoding itself.
            let ext_size = maybe_unsynch_u32(BigEndian::read_u32(&ext_header[0..4]));
            dbg!(ext_size);
            if ext_size != 6 && ext_size != 10 {
                return Err(Error::new(
                    ErrorKind::Parsing,
                    "extended tag header size must be either 6 or 10",
                ));
            }
            let ext_flags = ExtFlags::from_bits(BigEndian::read_u16(&ext_header[4..6]))
                .ok_or_else(|| {
                    Error::new(
                        ErrorKind::Parsing,
                        "unknown tag extended header flags are set",
                    )
                })?;
            //let size_of_padding = maybe_unsynch_u32(BigEndian::read_u32(&ext_header[6..10]));

            let crc = if ext_flags.contains(ExtFlags::CRC) {
                Some(maybe_unsynch_u32(reader.read_u32::<BigEndian>()?))
            } else {
                None
            };
            Some(ExtHeader {
